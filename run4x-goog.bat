...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '%~dp0run_grid2.ps1' -Funds GOOGL  -DataSuffix '-5Y' -Population 4 -Generations 2 -GaSearchPreset grid -PriceColumn 'Adj Close' -LogFile 'goog-run4x.log' %*"
rem powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '%~dp0run_grid.ps1' -Funds AMZN  -DataSuffix '-5Y' -Population 6 -Generations 3 -GaSearchPreset grid -PriceColumn 'Adj Close' -LogFile 'amzn-run6.log' %*"
