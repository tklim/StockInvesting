...
rem powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '%~dp0run_grid.ps1' -Funds NVDA  -DataSuffix '-5Y' -Population 4 -Generations 2 -GaSearchPreset grid -PriceColumn 'Adj Close' -LogFile 'nvda-run4.log'  %*"
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '%~dp0run_grid.ps1' -Funds NVDA  -Population 4 -Generations 2 -GaSearchPreset grid -PriceColumn 'Adj Close' -LogFile 'nvda-run4.log'  %*"
