...
set "LOGFILE=runlogs\%ticker%-run%pop%x.log"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '%~dp0run_grid2.ps1' -Funds %ticker_upper% -DataSuffix '-%year%Y' -TransitionPolicy grandfather -Population %pop% -Generations %gen% -GaSearchPreset grid -PriceColumn 'Adj Close' -LogFile '%LOGFILE%' %EXTRA_ARGS%"
